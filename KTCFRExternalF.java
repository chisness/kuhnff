...
public class KTCFRExternalF {
    ...
    public void train(int iterations, int decksize, int buckets) {
        ...
        for (int i = 0; i < iterations; i++) {
            ...
            util0 += cfr(cards, "", 0, decksize, starttime, i, buckets);
            util1 += cfr(cards, "", 1, decksize, starttime, i, buckets);
        }
        System.out.println("Average game value: " + util0 / iterations);
        for (Node n : nodeMap.values())
            System.out.println(n);
    }
    

    private double cfr(int[] cards, String history, int player_iteration, int decksize, long starttime, int currit, int buckets) {
    ...
    if ((nodecount == 100000) || (nodecount == 1000000) || (nodecount == 10000000) || (nodecount == 100000000) || (nodecount == 1000000000) || (nodecount % 100000000)==0) {//|| (nodecount == 10000000000)) {
        ...
        if (player != player_iteration) {
          for (int a = 0; a < NUM_ACTIONS; a++) {
            node.strategySum[a] += node.strategy[a];
          }
          double dart = Math.random();
          String nextHistory = history + (dart < strategy[0] ? "p" : "b");
          return cfr(cards, nextHistory, player_iteration, decksize, starttime, currit, buckets);
        }
        
        if (player == player_iteration) {
          for (int a = 0; a < NUM_ACTIONS; a++) {
            String nextHistory = history + (a == 0 ? "p" : "b");
            util[a] = cfr(cards, nextHistory, player_iteration, decksize, starttime, currit, buckets);
            nodeUtil += strategy[a] * util[a]; //weighted utility for the node
          }
          for (int a = 0; a < NUM_ACTIONS; a++) {
            double regret = util[a] - nodeUtil;
            node.regretSum[a] += regret; //store regretsum for each node->action
          }
          return nodeUtil;
        }
        return 0;
  ...
  return brf(player_card, dummyHistory, player_iteration, oppreach); //give opponent player dummy card of 1 that is never used
    ...
    public static void main(String[] args) {
        ...
        new KTCFRExternalF().train(iterations, decksize, buckets);
    }
