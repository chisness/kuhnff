...
public class KTCFRVanillaPlusF {
    ...
    class Node {
        String infoSet;
        double[] regretNew = new double[NUM_ACTIONS], 
        ...
        private double[] getStrategy(double realizationWeight) {
            ...
            for (int a = 0; a < NUM_ACTIONS; a++) {
                strategy[a] = regretNew[a] > 0 ? regretNew[a] : 0;
        ...
        public String toString() {
                return String.format("%4s: %s", infoSet, Arrays.toString(getStrategy(-1)));
        }
        ...
        int ctr = 0;
         ...
         for (int i = 0; i < iterations; i++) { //this makes sense to repeat for each iteration
           ...
           for (Node n : nodeMap.values()) {
             ...
             n.regretNew[0] += Math.max(n.regretSum_t[0], 0);
             n.regretNew[1] += Math.max(n.regretSum_t[1], 0);
             n.regretSum_t[0] = 0;
             n.regretSum_t[1] = 0;
             //System.out.println(n.regretSum[0]);
           }
    ...
    private double cfr(int[] cards, String history, double p0, double p1, int decksize, long starttime, int currit) {  
        ...
        for (int a = 0; a < NUM_ACTIONS; a++) {
            ...
            //node.regretNew[a] += (player == 0 ? p1 : p0) * regret;
            //node.regretNew[a] = Math.max(0, node.regretNew[a]);
        }
  ...
  return brf(player_card, dummyHistory, player_iteration, oppreach); //give opponent player dummy card of 1 that is never used
    ...
    public static void main(String[] args) {
        ...
        int decksize = 3;
        new KTCFRVanillaPlusF().train(iterations, decksize);
    }
